...
class ImageService(object):
    ...
    def resize(img, width: int, height: int):
        ...
        If width is None it is calculated using the ratio between the old and new height
        in order to maintain the original height/width ratio. Same for height, if it is none.
        If both width and height are specified, we will use the number that has the highest ratio.
        """
        width, height = ImageService.calculate_w_h(img.width, img.height, width, height)
